...
const render = (tasks) => {
  Tasks.innerHTML = "";
  tasks.map((task) => {
    if (task) Tasks.insertAdjacentHTML("beforeend", cardMarkup(task));
  });
};
...
searchIn.addEventListener("input", () => {
  ...
  const results = myTasks.filter((task) =>
...
const quoteApiKey = "JqQn0J7OYYw/EySiMDcV2A==udT3h9AQEcyFCPBn";
...
async function fetchQuote() {
  try {
    ...
    document.querySelector(".author").textContent = data[0].author || "Unknown";
    ...
    document.querySelector(".quote").style.color = "pink";
  }
}
